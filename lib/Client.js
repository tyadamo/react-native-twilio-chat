...
class Client {
  ...
  getSubscribedChannels() {
    return TwilioChatChannels.subscribedChannels()
  ...
  register(token, type) {
    if (!type) {
      ...
      TwilioChatClient.register(token, type);
    }    
  }

  unregister(token, type) {
    if (!type) {
      TwilioChatClient.unregister(token);
    } else {
      TwilioChatClient.unregister(token, type);
    }
  }

  registerPushNotification = (token) => {
    return TwilioChatClient.registerNotificationToken(token)
  }

  unRegisterPushNotification = (token) => {
    return TwilioChatClient.unRegisterNotificationToken(token)
  }
